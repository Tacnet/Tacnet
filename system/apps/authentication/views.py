...
def register(request):
    ...
    if request.method == "POST":
        ...
        if username == "" or password == "" or retypepassword == "" or password != retypepassword:
            return HttpResponse(False)
...
def forgot_password(request):
    ...
    if request.method == 'POST':
        try:
            ...
            currentUser = User.objects.get(email=mail)



            newpw = ""''.join(random.choice(string.ascii_uppercase + string.digits) for x in range(8))

            currentUser.set_password(newpw)

            html_content = "<h3>User Restore</h3>" \
                           "<p>New password: " + newpw + "</p>"

            msg = EmailMessage('[' + 'Tacnet.io' + '] User Restore', html_content, 'no-reply@tacnet.io', [currentUser.email])
